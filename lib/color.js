...
function modeStringAddBg(str) {
  // 权限的背景色
  return chalk.bgHex('#121212')(' ' + str + ' ')
}

function modeColor(str, type) {
  // 权限
  let strArr = str.split('')

  if(type === 'link') {
    strArr[0] = chalk.hex('#61D6D6')('l')
  } else {
    switch (strArr[0]) {
      case 'd':
        strArr[0] = chalk.hex('#005FD7')('d')
        break;
      case '-':
        strArr[0] = chalk.hex('#8A8A8A')('-')
        break;
      case '.':
        strArr[i] = chalk.hex('#13A10E')('.')
        break;
    }
  }
  for(let i = 0; i < strArr.length; i++) {
    switch (strArr[i]) {
      case 'r':
        strArr[i] = chalk.hex('#13A10E')('r')
        break;
      case 'w':
        strArr[i] = chalk.hex('#C19C00')('w')
        break;
      case 'x':
        strArr[i] = chalk.red('x')
        break;
      case '-':
        strArr[i] = chalk.hex('#8A8A8A')('-')
        break;
    }
  }
  return strArr.join('')
