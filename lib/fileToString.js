...
function noList(filesStats, isAddBgColor) {
...
function addIndent(filesStats, argvs) {
  for(let fileStats of filesStats) {
    ...
    //     let flag = preIndent[i] - (preIndent[i - 1] || 0) !== 1
    //     if(flag) {
    //       preStr += ''.padStart(preIndent[i] - (preIndent[i - 1] || 1))
    //     }
    //     preStr += '│' + ''.padStart(2)
    ...
    // }
  fileStats.string = ''.padStart((argvs.indent) * 2) + fileStats.string
  }
}
