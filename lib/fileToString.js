...
function addAllFormat(filesStats, isAddBgColor, isIcon) {
  ...
  for(let fileStats of filesStats) {
    ...
    fileStats.string = `${isAddBgColor ? color.modeStringAddBg(modeColorString) : modeColorString} ${fileStats.username} ${fileStats.groupname} ${fileStats.size} ${fileStats.time} ${fileStats.string}`
