...
function gitStatusFilter(filesStats, gitStatus) {
  for(let fileStats of filesStats) {
    let fileFullPathName = fileStats.fullPathName
    if(!gitStatus[fileFullPathName]) continue ;
    if(gitStatus[fileFullPathName][0] === ' ') {
      if(gitStatus[fileFullPathName][1] !== ' ') {
        fileStats.string += ' ' + gitStatusIcon(gitStatus[fileFullPathName][1])
      }
    } else {
      fileStats.string += ' ' + gitStatusIcon(gitStatus[fileFullPathName][0])
    }
  }
}
...
function fileToString (filesStats, argvs, isIgnores, gitStatus) {
  ...
  if(argvs.gitStatus) {
    gitStatusFilter(filesStats, gitStatus)
  }
