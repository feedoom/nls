...
function stringLength(str) {
  // 获取字符串再屏幕上显示的真实长度，排除中文字
  let otherStringCount = [...(str + '').matchAll(/[^\x00-\xff]/g)].length
  return str.length - otherStringCount + otherStringCount * 2
}

function padSpace(str, num) {
  // 用空格填充字符串
  return str + ' '.repeat(num - stringLength(str))
}
...
function joinString(fileStringArr) {
  ...
  for(let i = 0; i < fileStringArr[0].length; i++) {
    ...
    for(let j = 0; j < fileStringArr.length; j++) {
      let currentStringLength = fileStringArr[j][i] ? stringLength(fileStringArr[j][i]) : 0
  ...
  fileStringArr.forEach(currentRowFiles => {
    ...
    for(let i = 0; i < currentRowFiles.length; i++) {
      let currentFile = padSpace(currentRowFiles[i], maxColumns[i])
      str += currentFile
      spaceArr.push(' '.repeat(stringLength(currentFile) - stringLength(currentRowFiles[i])))
    }
    joinString.push(str + '\n')
...
function addSpace(filesStats, isAddBgColor) {
  ...
  for(let i = 0; i < filesStats.length; i++) {
    filesStats[i].string = fileColor(filesStats[i].file, filesStats[i].string, isAddBgColor) // + spaceArr[i]
  }
}
